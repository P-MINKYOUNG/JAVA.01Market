...
public class Market {
	...
	int sumAll;
//	momchance
	int momMoney = (int)((Math.random()*20)+10)*1000;
	...
	public int finalMoney() {
		...
		int minusMoney = money - sumAll + momMoney;
	...
	public void momChance() {
		...
		System.out.println("엄마찬스를 원하시나요? ( Y / N )");
		
		while (true) {
			char answer = sc.next().charAt(0);
			answer = Character.toUpperCase(answer);
			if(answer == 'Y') {
				//엄마의 momMoney를 랜덤으로 대입해주는 공식
				System.out.println(momMoney);
				break;
			} else if (answer == 'N') {
				// 반복문을 빠져나가는 공식
				break;
